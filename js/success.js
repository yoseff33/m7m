...
async function loadOrderDetails(transactionNo) {
    try {
        ...
        // إذا كان الطلب مدفوعاً ولم يتم تعيين كود بعد، جرب تعيين كود
...
function displayOrderDetails(order) {
    ...
    if (order.activation_codes && order.activation_codes.length > 0) {
        const code = order.activation_codes[0].code;
        activationCode = `
    ...
    orderDetails.innerHTML = `
        <div class="details-card hud-effect" style="padding: 30px; background: rgba(26, 26, 26, 0.8); border-radius: 16px; border: 1px solid rgba(255, 215, 0, 0.1);">
            ...
            <div class="detail-item" style="margin-bottom: 25px;">
                ...
                ${order.discount > 0 ? `
                    <div style="color: #2ecc71; margin-top: 5px;">
                        <i class="fas fa-tag"></i> شامل خصم ${(order.discount / 100).toFixed(2)} ر.س
                    </div>
                ` : ''}
            </div>
            ...
            <div class="action-buttons" style="margin-top: 30px; display: flex; gap: 15px; flex-wrap: wrap;">
                ...
                ${order.status === 'completed' && order.activation_codes && order.activation_codes.length > 0 ? `
